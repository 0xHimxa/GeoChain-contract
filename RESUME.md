...
Protocol engineer specializing in prediction market design and cross-chain systems. Built an LMSR-based pricing engine on EVM using hybrid off-chain computation with on-chain invariant enforcement, enabling probability-consistent markets with bounded loss and safe cross-chain execution.
Designed and implemented GeoChain, an autonomous cross-chain prediction market protocol with dual-path price synchronization and non-custodial agent execution.
