will need to make it posible market facotry to buy to it can try to fix price if it getting messy


1) succefully setUp cron to check factory balance for refill
 2)  implemented a Gemini helper function for  creating, resloving and checking for dublicate event
  did created a function that will all worlkflow to write to db




1)  things not done yet: a function that create new event on supporteds chains onchain  and also store it off-chain
2)  function that will reslove event if so call the reslove function, and broadcast it on other chains
 
3) function  that compare but chain price  if any is bigger it call the broadcast function fpr sync

4) and also a function the check price different for arbitrage fixing
