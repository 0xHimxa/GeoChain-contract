...
const arbitrateUnsafeMarketHandler = (runtime: Runtime<Config>): string => {
  ...
  for (const evmConfig of runtime.config.evms) {
    ...
    for (const marketAddress of activeMarketList) {
      ...
      const deviationResult = evmClient.callContract(runtime, {
        call: encodeCallMsg({
          from: sender,
          to: marketAddress,
      ...
      const encodedReport = encodeAbiParameters(
        parseAbiParameters("string actionType, bytes payload"),
        ["priceCorrection", correctionPayload]
      );
      const reportResponse = runtime.report({
        ...prepareReportRequest(encodedReport),
      }).result();
      ...
      if (writeReportResult.txStatus === TxStatus.REVERTED) {
        runtime.log(`[${evmConfig.chainName}] priceCorrection REVERTED for marketId=${marketId}: ${writeReportResult.errorMessage || "unknown"}`);
        continue;
      }
