...
var signUpWorkFlow = (runtime2) => {
  ...
  return httpClient.sendRequest(runtime2, authRequester, consensusIdenticalAggregation())().result();
};
...
var asStringArray = (field) => {
  if (!field || !("arrayValue" in field))
    return [];
  const values = field.arrayValue.values || [];
  return values.map((value2) => asString(value2)).filter((value2) => typeof value2 === "string");
};
...
var getProjectId = (runtime2) => {
  return runtime2.getSecret({ id: "FIREBASE_PROJECT_ID" }).result().value;
};
var toSessionDocFields = (session) => ({
...
var parseSessionDoc = (doc) => {
  if (!doc?.fields || !doc.updateTime)
    return null;
  const fields2 = doc.fields;
  const sessionId = asString(fields2.sessionId);
  const owner = asString(fields2.owner);
  const sessionPublicKey = asString(fields2.sessionPublicKey);
  const chainId = asInteger(fields2.chainId);
  const maxAmountUsdc = asString(fields2.maxAmountUsdc);
  const expiresAtUnix = asInteger(fields2.expiresAtUnix);
  const revoked = asBoolean(fields2.revoked);
  ...
  return {
    ...
    allowedActions: asStringArray(fields2.allowedActions).sort(),
    maxAmountUsdc,
    expiresAtUnix,
    revoked,
...
var getSessionDoc = (runtime2, idToken, sessionId) => {
  ...
  return parseBodyText(response.bodyText);
};
var createSessionDoc = (runtime2, idToken, session) => {
  const projectId = getProjectId(runtime2);
...
var getFirestoreIdToken = (runtime2) => {
  ...
  if (!auth?.idToken)
    throw new Error("firebase sign-up did not return idToken");
  return auth.idToken;
};
var sessionsEqual = (a, b) => {
  return a.sessionId === b.sessionId && a.owner.toLowerCase() === b.owner.toLowerCase() && a.sessionPublicKey.toLowerCase() === b.sessionPublicKey.toLowerCase() && a.chainId === b.chainId && a.maxAmountUsdc === b.maxAmountUsdc && a.expiresAtUnix === b.expiresAtUnix && [...a.allowedActions].sort().join(",") === [...b.allowedActions].sort().join(",");
};
var upsertAndValidateSession = (runtime2, idToken, session) => {
  const existing = getSessionDoc(runtime2, idToken, session.sessionId);
  if (!existing) {
    const created = createSessionDoc(runtime2, idToken, session);
    if (created)
      return { ok: true };
    const reloaded = getSessionDoc(runtime2, idToken, session.sessionId);
    const reloadedSession = parseSessionDoc(reloaded);
    if (!reloadedSession)
      return { ok: false, reason: "stored session state is malformed" };
    if (reloadedSession.revoked)
...
var reserveSessionNonce = (runtime2, idToken, sessionId, nonce, intentHash) => {
  const projectId = getProjectId(runtime2);
  const url = `${baseUrl(projectId)}/${SESSIONS_COLLECTION}/${encodeURIComponent(sessionId)}/used_nonces?documentId=${encodeURIComponent(nonce)}`;
  const response = sendFirestoreRequest(runtime2, idToken, {
    url,
    method: "POST",
    body: {
      fields: {
        ...
        createdAtUnix: { integerValue: String(Math.floor(runtime2.now().getTime() / 1000)) }
      }
    }
  });
  if (response.statusCode === 200)
...
var createApprovalRecord = (runtime2, idToken, approval) => {
  const projectId = getProjectId(runtime2);
...
var getApprovalDoc = (runtime2, idToken, approvalId) => {
  const projectId = getProjectId(runtime2);
...
var markApprovalUsed = (runtime2, idToken, approvalId, updateTime) => {
  const projectId = getProjectId(runtime2);
  ...
  const response = sendFirestoreRequest(runtime2, idToken, {
    ...
    body: {
      fields: {
        ...
        usedAtUnix: { integerValue: String(Math.floor(runtime2.now().getTime() / 1000)) }
      }
    }
  });
  return response.statusCode === 200;
};
var consumeApprovalRecord = (runtime2, idToken, expected) => {
  const doc = getApprovalDoc(runtime2, idToken, expected.approvalId);
  if (!doc)
    return { ok: false, reason: "approval does not exist" };
  const stored = parseApprovalDoc(doc);
  if (!stored)
    return { ok: false, reason: "approval state is malformed" };
  if (stored.used)
  ...
  const sessionDoc = getSessionDoc(runtime2, idToken, stored.sessionId);
  if (!sessionDoc)
    return { ok: false, reason: "session not found for approval" };
  const session = parseSessionDoc(sessionDoc);
  if (!session)
    return { ok: false, reason: "stored session state is malformed" };
  if (session.revoked)
    return { ok: false, reason: "session revoked" };
  if (session.expiresAtUnix < expected.nowUnix)
    return { ok: false, reason: "session expired" };
  const marked = markApprovalUsed(runtime2, idToken, expected.approvalId, stored.updateTime);
  if (!marked)
...
var createSessionEip712Domain = (chainId) => ({
  name: SESSION_EIP712_NAME,
  version: SESSION_EIP712_VERSION,
  chainId
});
var normalizeActions = (actions) => {
...
var toLower = (value2) => value2.toLowerCase();
var isAddressEqual2 = (a, b) => toLower(a) === toLower(b);
var addressFromPublicKey = (publicKey) => {
...
var validateSessionAuthorization = async (runtime2, input, existingFirestoreToken) => {
  ...
  if (!session)
    return { ok: false, reason: "missing session authorization" };
  ...
  if (!NONCE_REGEX.test(requestNonce))
    return { ok: false, reason: "invalid session.requestNonce" };
  ...
  const allowedActions = Array.isArray(session.allowedActions) ? session.allowedActions.map((action) => String(action).trim()).filter(Boolean) : [];
  ...
  if (maxAmountUsdc === null)
    return { ok: false, reason: "session.maxAmountUsdc must be uint string" };
  ...
  if (expiresAtUnix === null)
    return { ok: false, reason: "session.expiresAtUnix must be uint" };
  const nowUnix = BigInt(Math.floor(runtime2.now().getTime() / 1000));
  if (expiresAtUnix < nowUnix)
    return { ok: false, reason: "session expired" };
  const policyMaxSessionDurationSec = runtime2.config.sponsorPolicy?.sessionMaxDurationSec || 86400;
  if (expiresAtUnix > nowUnix + BigInt(policyMaxSessionDurationSec)) {
    return { ok: false, reason: "session duration exceeds policy" };
  }
  if (!isAddressEqual2(input.sender, owner)) {
  ...
  if (!grantSigOk)
    return { ok: false, reason: "invalid session grant signature" };
  const sessionAddress = addressFromPublicKey(sessionPublicKey);
  if (!sessionAddress)
    return { ok: false, reason: "invalid session public key" };
  const intentMessage = {
  ...
  if (!requestSigOk)
    return { ok: false, reason: "invalid session request signature" };
  const firestoreToken = existingFirestoreToken || getFirestoreIdToken(runtime2);
  const storeSessionResult = upsertAndValidateSession(runtime2, firestoreToken, {
    sessionId,
  ...
  const nonceResult = reserveSessionNonce(runtime2, firestoreToken, sessionId, requestNonce, intentHash);
  if (!nonceResult.ok) {
...
var makeDecision = (requestId, validationFailedReason) => {
...
var sponsorUserOpPolicyHandler = async (runtime2, payload) => {
  ...
  if (!policy?.enabled) {
    return JSON.stringify({
      ...
      requestId: `req_${runtime2.now().toString()}`
    });
  }
  ...
  const requestId = request.requestId || `req_${runtime2.now().toString()}`;
  ...
  const expectedAgentActionType = AGENT_ACTION_TO_ROUTER_ACTION_TYPE[request.action];
...
var executeReportHttpHandler = async (runtime2, payload) => {
  ...
  let amount;
  try {
    amount = toBigIntAmount2(req.amountUsdc);
  } catch (error) {
    return JSON.stringify({
      submitted: false,
      requestId,
  ...
  if (!req.payloadHex || !HEX_BYTES_REGEX2.test(req.payloadHex)) {
    return JSON.stringify({
      submitted: false,
      requestId,
...
var ethCreditFromLogsHandler = (runtime2, log) => {
  ...
  if (!HEX_ADDRESS_REGEX6.test(sender)) {
  ...
  runtime2.log(`[ETH_CREDIT] sender=${sender} amountWei=${amountWei.toString()} amountUsdcE6=${amountUsdcE6.toString()} txHash=${txHashHex} logIndex=${log.index}`);
