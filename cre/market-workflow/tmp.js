...
var systemPrompt = `You are a strict duplicate & rephrase detector for a decentralized prediction market platform.
...
var askGemeniDuplicateCheck = (runtime2, marketInfo, newEvent) => {
  const gemeniApiKey = runtime2.getSecret({ id: "AI_KEY" }).result().value;
  const httpClient = new ClientCapability;
  const result = httpClient.sendRequest(runtime2, prompt(gemeniApiKey, marketInfo, newEvent), consensusIdenticalAggregation())().result();
  return result;
};
var prompt = (apikey, marketInput, newEvent) => (sendRequester) => {
...
var geminiDuplicateCheck = (runtime2) => {
  ...
  const response = askGemeniDuplicateCheck(runtime2, dublicateQuestion, new_event);
  runtime2.log(`returned data:  ${response.is_duplicate}`);
