...
var BRIDGE_BALANCE_THRESHOLD = 500000n * USDC_DECIMALS;
var BRIDGE_TOP_UP_AMOUNT = 540000n * USDC_DECIMALS;
var ROUTER_BALANCE_THRESHOLD = 50000n * USDC_DECIMALS;
var ROUTER_TOP_UP_AMOUNT = 3000000n * USDC_DECIMALS;
var FACTORY_BALANCE_THRESHOLD = 310000n * USDC_DECIMALS;
var FACTORY_TOP_UP_AMOUNT = 600000n * USDC_DECIMALS;
...
var userPrompt = `Resolve the market strictly from authoritative evidence.
...
var askGemeniResolve = (runtime2, marketInfo) => {
  ...
  return httpClient.sendRequest(runtime2, prompt(gemeniApiKey, marketInfo, currentTimeIso), consensusIdenticalAggregation())().result();
};
var prompt = (apikey, marketInput, currentTimeIso) => (sendRequester) => {
  const dataToSend = {
...
var syncCanonicalPrice = (runtime2) => {
  ...
  for (const marketAddress of activeMarketList) {
    ...
    const syncSnapshotResult = hubClient.callContract(runtime2, {
      call: encodeCallMsg({
        from: sender,
        to: marketAddress,
...
DISPUTED_OUTCOME_SET: ${JSON.stringify(input.disputedOutcomes)}`
  ...
  const parsed = JSON.parse(rawData);
  const aiResponseString = parsed?.candidates?.[0]?.content?.parts?.[0]?.text;
  return parseGeminiJson2(aiResponseString);
};
